        ...
        <div v-for="borrow in borrowList" :key="borrow.id" class="borrow-card">
          ...
          <div class="borrow-info">
            ...
            <div class="borrow-details">
              ...
              <p><span class="label">状态:</span> <span :class="'status-' + getStatusClass(borrow.status)">{{ getStatusText(borrow.status) }}</span></p>
            </div>
            <div v-if="borrow.status === 1" class="borrow-tip">
              <el-alert
                title="借阅申请正在审核中，请耐心等待"
                type="info"
                :closable="false"
                show-icon
              />
            ...
            <div v-if="borrow.status === 2 && isAboutToExpire(borrow.planReturnTime)" class="borrow-tip">
              <el-alert
                :title="`该书将于 ${formatDateTime(borrow.planReturnTime)} 到期，请及时归还或续借`"
                type="warning"
                :closable="false"
                show-icon
              />
            ...
            <div class="borrow-actions">
              <el-button 
                ...
                type="danger" 
                class="cancel-btn"
                @click="handleCancel(borrow)"
              >
                取消申请
              </el-button>
              <el-button 
                v-if="borrow.status === 2" 
                type="primary" 
                class="return-btn"
              ...
              <el-button 
                v-if="borrow.status === 2 && borrow.renewCount < maxRenewCount" 
                type="success" 
                class="renew-btn"
                @click="handleRenew(borrow)"
              >
                续借
              </el-button>
              <el-button 
...
:deep(.el-tabs__active-bar) {
  height: 3px;
  border-radius: 3px;
  background: linear-gradient(90deg, #409eff 0%, #5352ed 100%);
}
